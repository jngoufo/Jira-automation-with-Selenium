...
from utilities.locators import ProjectPageLocators
...
class Projectpage(BaseDriver):
    ...
    def viewproject(self, project):
        ...
        self.wait_for_presence_of_all_elements(ProjectPageLocators.customerservice_project)
        print(Style.BRIGHT + f"la page '{self.driver.title}' est chargée")
        project_names_list = self.driver.find_elements(By.CSS_SELECTOR, ProjectPageLocators.customerservice_project)
        ...
        try:
            if not project_names_list:
                ...
                assert cs in projects, Fore.RED + f"Element '{cs}' was not found in the list of projects"
                print(Fore.GREEN + f"Test passed: Element '{cs}' is in the list of projects.")
                print(Style.RESET_ALL)
        except TimeoutException:
            print(
                Fore.CYAN + f"Timeout : Les éléments avec le sélecteur '{selecteur_customerserviceissues}' n'ont pas été trouvés dans le temps imparti.")
            print(Style.RESET_ALL)
